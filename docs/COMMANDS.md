...
- `alarm bundle plan|dry-run|apply|rollback|status` 支持把 topic、integration、notice_content、notice、policy 组合成可执行图。每个资源可独立选择 `existing`、`create`、`update`、`skip`，因此可复用客户已有 topic、集成配置或通知渠道组；`dry-run` 会在本地解析 token、校验最终 payload，并输出不写云资源的写入步骤与回滚预览；`apply` 会把前序资源返回的 ID 注入后续 payload 并输出 manifest，`rollback` 只删除 manifest 中 `mode=create` 的资源。
