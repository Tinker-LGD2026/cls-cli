...
cls alarm policy scaffold --scenario http-5xx --name 'api 5xx' --logset-id logset-xxx --topic-id topic-xxx --threshold 10 --window-minutes 5 --monitor-period 1 --alarm-period 15 --notice-id notice-xxx
...
cls alarm bundle apply --bundle @bundle.json --confirm-real-write --confirmation-text 同意执行 --manifest-out manifest.json --region ap-guangzhou
...
- `alarm discover integration|notice|notice-content|policy --name ...` 只读发现已有告警资源，返回 `selected`、`matches`、`ambiguous`、`request_ids`；策略发现可加 `--topic-id` 缩小范围，歧义时默认退出 1，确认需要多结果时加 `--allow-multiple`。`list` 默认按 `--offset 0 --limit 20` 拉取单页，并在 JSON 中输出 `total_count`、`fetched_count`、`truncated`、`page_count` 和 `request_ids`；需要拿全量时使用 `--all`，避免 Agent 把第一页误判为全量。
...
- `alarm policy scaffold` 支持两种模式：不传 `--query/--condition` 时生成内置 `http-5xx` 示例策略；传入 `--query` 和 `--condition` 时使用通用模式，把 `cls ai generate-query` 或人工确认后的检索分析语句组装成策略 payload。时间参数拆成三类：`--window-minutes` 或 `--start-time-offset/--end-time-offset` 控制查询窗口，默认执行周期 `--monitor-period=1` 分钟，默认告警频率 `--alarm-period=15` 分钟；如果客户说“告警频率 60 分钟一次”，传 `--alarm-period 60`，不要改查询窗口或执行周期。高级策略可用 `--multi-condition-expr/--multi-condition-level` 生成 `MultiConditions`，用 `--group-by` 生成分组触发字段，用 `--callback-body/--callback-header` 生成策略级 `CallBack`；浏览器真实验证过的 `--monitor-type fixed|cron`、`--cron-expression`、`--analysis-query`、`--analysis-original-fields`、`--classification key=value` 可生成固定/cron 调度、多维分析和告警分类；复杂字段仍可用 `@file.json` 透传。生成 `MultiConditions` 时会按官方互斥规则省略 `Condition/AlarmLevel`。
- `alarm policy validate` 校验 `AlarmTargets[].Query`、`Condition` 与查询 alias 的一致性，并对 `MultiConditions`、`GroupTriggerStatus`、`GroupTriggerCondition`、`Analysis[]`、`Classifications`、`MessageTemplate`、`CallBack`、`MonitorTime` 做 schema-lite 类型校验。字段校验说明：CLS `CreateIndex` 文档未给出完整字段名字符集约束，CLI 对本地生成的 SQL alias 使用严格标识符规则，并对 `--fields`、`--group-by`、`--analysis-original-fields` 等由 CLI 拼入 payload/query 的字段使用保守白名单 `^[A-Za-z_][A-Za-z0-9_.$-]{0,127}$`；不拒绝用户显式传入的原始 `--query`，原始查询应通过 `alarm policy test-query` 做执行验证。
- `alarm validate bundle` 聚合校验策略、通知内容模板、通知渠道组和集成配置，并检查 `AlarmNoticeId`、`NoticeContentId`、`WebCallbackId` 的绑定关系。
- `alarm bundle plan|dry-run|apply|rollback|status` 支持把 topic、integration、notice_content、notice、policy 组合成可执行图。每个资源可独立选择 `existing`、`create`、`update`、`skip`，因此可复用客户已有 topic、集成配置或通知渠道组；`dry-run` 会在本地解析 token、校验最终 payload，并输出不写云资源的写入步骤与回滚预览；`apply` 会把前序资源返回的 ID 注入后续 payload 并输出 manifest，真实写入必须同时传 `--confirm-real-write --confirmation-text 同意执行`，建议总是加 `--manifest-out manifest.json` 保存回滚依据；`rollback` 只删除 manifest 中 `mode=create` 的资源，显式回滚必须传 `--force --confirmation-text 同意执行`。
